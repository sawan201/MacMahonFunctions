...
(The best possible case is that $k_i=1$ for all $i$, i.e., the bases are orthonormal.  However, we're not always that lucky.)
...
\item $\Mac_u$ = MacMahon symmetric functions of homogeneous multidegree $u$
...
\end{equation}
The idea is to use this for basis transitions, along the lines discussed earlier.  Specifically, formula~\eqref{expand-in-basis} says that for any $F\in\Mac_u$, we have
...
Moreover, for all $f,g\in\Mac_u$ we have

...
Let $\uu\in\Nn^r$.  A \textbf{vector partition} of $\uu$ is a multiset of nonzero vectors $\lambda=\lambda^{(1)},\dots,\lambda^{(k)}\in\Nn^r$ that sum to $\uu$.
 
\begin{itemize}
%\item The $\lambda^{(j)}$ are the \textbf{parts} of $\lambda$.
\item The \textbf{weight} $wt(\lambda)$ is the sum of all entries in its vectors.

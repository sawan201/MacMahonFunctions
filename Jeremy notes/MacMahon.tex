...
\section{Vector partitions and numerical functions}

Let $\Nn$ denote the nonnegative integers and $\Pp$ the positive integers.

Let $\uu\in\Nn^r$.  Write $\uu=(u_1,\dots,u_r)$ and $\wt(\uu)=\sum u_i$.  A \textbf{vector partition} of $\uu$ is a multiset of nonzero vectors $\lambda=\lambda^{(1)},\dots,\lambda^{(k)}\in\Nn^r$ that sum to $\uu$.
 
\begin{itemize}
\item The \textbf{length} is $\ell(\lambda)=k$.
\item The \textbf{weight} $wt(\lambda)$ is the sum of all entries in its vectors (so $\wt(\lambda)=\wt(\uu)$).
\item The \textbf{sign} is $\sign(\lambda)=(-1)^{\text{number of vectors with even sum}}$
\item For a vector partition $\lambda$ in which each vector $v_i$ occurs with multiplicity $m_i$, define
\[|\lambda|=\prod_im_i! \qqandqq \lambda!=\prod_i\prod_{x\in v_i}(x!)^{m_i}.\]
\end{itemize}

For example, the vector partitions of $\uu=(2,1)=21$ are as follows.
\[\begin{array}{cccc}
\lambda & \sign(\lambda) & |\lambda| & \lambda!\\ \hline
21 & +1 & 1 & 2\\
20,01 & -1 & 1  & 2\\
10,11 & -1 & 1 & 1\\
10,10,01 & +1 & 2 & 1 
...
A vector partition is \textbf{unitary} if it is a partition of the all-ones vector $\mathbf{1}\in\Nn^k$.  There is an obvious bijection between unitary vector partitions and set partitions of $[k]$.
...
\section{MacMahon symmetric functions}

Fix $r\in\Pp$ and let $\mathcal{A}=\{x_{i,j} \mid i\in[r],\ j\in\Pp\}$.
...
Following Rosas \cite{Rosas}, the standard combinatorial bases for $\Mac$ are as follows.  For $\uu\in\Nn^r$, each basis for $\Mac_\uu$ is indexed by the vector partitions of~$\uu$.
...
The unitary MacMahon symmetric functions are the graded piece $\Mac_{(1)^r}$.  These are the focus of \cite{Doubilet}.
 
\section{Transitions between bases}

First, a standard linear algebra fact.
Let $V$ be a vector space of finite dimension~$n$ over ground field $\Bbbk$
...
&=  \frac{\uu!}{\binom{\uu}{\mu}} \sum_{\lambda\partn\uu} \frac{\#\{(\pi,\sigma)\mid \type_\uu(\pi)=\lambda,\ \type_\uu(\sigma)=\mu,\ \sigma\leq\pi\}}{\binom{\uu}{\lambda}\lambda!}
    m_\lambda
\end{align*}
...
\end{equation}
where $\mu(\hatzero,\lambda)$ means $\mu(\hatzero,\pi)$ for any set partition $\pi$ such that $\type_u(\pi)=\lambda$.  This can be verified using the lifting maps and Doubilet's formula.  For the purpose of computation, a set partition $\pi$ can be constructed as follows:
\begin{itemize}
\item Partition $[n]$ into sets $A_1,\dots,A_r$, with $|A_i|=u_i$.
\item For each vector $\lambda^{(i)}=(\lambda^{(i)}_1,\dots,\lambda^{(i)}_r)\in\lambda$, make a new block $B_i$ consisting of the $\lambda^{(i)}_j$ smallest elements of $A_j$, for each $j\in[r]$.  Remove those elements from $A_j$ when adding them to $B_i$.
\item At this point the sets $A_j$ should all be empty, and the desired set partition is $\pi=\{B_1,\dots,B_{\ell(\lambda)}\}$.
\end{itemize}


Let $A_1=[1,u_1]$, $A_2=[u_1+1,u_1+u_2]$, \dots $A_k=[u_1+\cdots+u_{k-1}+1,u_1+u_2]$, \dots $A_2=[u_1+1,u_1+u_2]$, \dots 
...
There is an involutive automorphism $\omega\colon\Mac\to\Mac$ (called $\theta$ in \cite{Doubilet}) defined by $\omega(e_\lambda) = h_\lambda$.  Rosas defines it at \cite[p.328]{Rosas} and then uses it to define the forgotten basis as $f_\lambda=\sign(\lambda)\omega(m_\lambda)$.  Rosas also states as a fact (in the middle of a proof on p.334) that $\omega(p_\lambda)=\sign(\lambda) p_\lambda$, although she does not explain where that formula comes from.  It may be in Doublet somewhere.  It certainly feels true (it's true for symmetric functions).  In summary, $\omega$ acts on basis elements by
\begin{align*}
\omega(e_\lambda) &= h_\lambda,\\
\omega(h_\lambda) &= e_\lambda,\\
\omega(m_\lambda) &= \sign(\lambda)f_\lambda,\\
\omega(f_\lambda) &= \sign(\lambda)m_\lambda,\\

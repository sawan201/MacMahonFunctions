...
The power-sum basis is orthogonal (though not orthonormal).  I would like to verify this, since it is not evident from going through the lifting map. A remark in Rosas \cite[p.333]{Rosas} says that for $\lambda\partn\uu$,
\begin{equation}
\langle p_\lambda,\,p_\mu \rangle = \frac{\delta_{\lambda,\mu}|\lambda|\,\lambda!}{|\mu(\hatzero,\lambda)|}
\end{equation}
where $\mu(\hatzero,\lambda)$ means $\mu(\hatzero,\pi)$ for any set partition $\pi$ such that $\type_u(\pi)=\lambda$.x1
% Verification:
%\begin{align*}
%\scp{p_\lambda}{p_\mu}
